...
public class pdpService extends Service {
	...
	public final static String ACTION_PDP_DECISION = "de.fraunhofer.iese.apppdp.pdpService";
	public final static String ACTION_PDP_SETPOLICY = "de.fraunhofer.iese.apppdp.setPolicy";
	public final static String ACTION_PDP_REVPOLICY = "de.fraunhofer.iese.apppdp.revPolicy";
	
	@Override
	public void onCreate() {
	    this.eventInformationFile = copyFileFromAssetsToInternalStorage(eventInformationFileName);
	    
	    this.decisionMessenger = new Messenger(new PDPDecisionHandler(getApplicationContext(),
	    		eventInformationFile));
	...
	private static class PDPDecisionHandler extends AbstractPolicyHandler {
		...
		public PDPDecisionHandler(Context context, File eventInformationFile) {
			...
			EventInformationParser eventInfoParser = new EventInformationParser
					(eventInformationFile.getAbsolutePath());
			Map<String, EventInformation> eventInformation =
					eventInfoParser.parseEventInformation();
		...
		public void handleMessage(Message msg) {
			...
			for (int i = 1; i < 20; i++) {
				...
				Decision d = lpdp.pdpNotifyEventJNI(event);
				Log.d("PDP", "xxx " + d.getAuthorizationAction().getType());
			}
			*/
			...
			Event event = reconstructEvent(
					msg.getData().getString("eventname"), msg);
			
			Decision d = lpdp.pdpNotifyEventJNI(event);
			
			Toast.makeText(context, "Event " + event.getEventAction()
					+ (d.getAuthorizationAction().getType() ? " allowed" : "inhibited"),
		...
		private Event reconstructEvent(String eventname, Message msg) {
			...
			for (Pair<Integer, String> parameterInformation : eventInfo
					.getParameterInformation()) {
				String paramName = parameterInformation.getRight();
				String paramValue = msg.getData().getString(
						"param" + parameterInformation.getLeft() + "value");

				event.addParam(new Param<String>(paramName, paramValue,
		...
		private Map<String, EventInformation> setupEventNameToInformationMap(
				Map<String, EventInformation> eventInfo) {
			...
			for (Map.Entry<String, EventInformation> entry : eventInfo.entrySet()) {
				eventNameToInfoMap.put(entry.getValue().getEventName(),
						entry.getValue());
			}
